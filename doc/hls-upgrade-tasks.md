...
- [ ] **Runtime-детект «modern не взлетел»**
  - [ ] В `index.html` после `document.write` modern-тега проверить
    `typeof Hls === 'undefined' || typeof Hls.isSupported !== 'function'`.
    Если true — записать `kp_legacy_hls=1` + `kp_legacy_hls_forced=1`
    в localStorage и `location.reload()`. Второй проход стартует в legacy.
  - [ ] Защита от цикла: если `kp_legacy_hls_forced` уже стоит — не дёргать
    modern, грузить legacy сразу.
  - [ ] Существующий UA-детект Chrome<47/webOS в загрузчике оставить как
    fast-path (чтобы не грузить modern-бандл заведомо зря), но это оптимизация.

- [ ] **`!Hls.isSupported()` в плеерах**
  - [ ] Сейчас `hls-engine.ts` и `tv-player.ts` не обрабатывают эту ветку —
    пользователь видит чёрный экран. Показывать ошибку через `showHlsError`
    / `errorView.showMessage`. Откатываться на legacy нет смысла — MSE нет
    ни в той, ни в другой версии.

- [ ] **Тост «устройство не тянет HEVC»**
  - [ ] В `hls-engine.ts` при `preferHevc === true` и серии `bufferStalledError`
    на hvc1-уровне (N штук подряд, порог подобрать) — показать одноразовый
    тост «Устройство не тянет HEVC, выключите в настройках».
  - [ ] Никакого автопереключения на avc1 — пользователь сам решает.

- [ ] **Скрытие пункта «Старый телевизор»** на forced-legacy платформах
  - [ ] Та же функция детекта из runtime-fallback, вынесенная в общий модуль.
    В `settings.ts` прятать пункт, если `kp_legacy_hls_forced` стоит.
