...
- [ ] **Настройка «Старый телевизор»**
  ...
  - [ ] Для известных legacy-платформ (Chrome < 47, webOS 1.x–2.x) настройка
...
- [ ] **Динамический загрузчик hls.js**
  ...
  - [ ] Инлайн-бутстрап в `index.html` (или `bootstrap.js`): читает
    `isLegacyHls()` из `localStorage` напрямую (без импорта `storage.ts`),
    применяет правила автодетекта, выбирает `hls-legacy.min.js` или
    `hls-modern.min.js`, создаёт `<script>` → `onload` подгружает основной `app.js`
  - [ ] Правила автодетекта:
    - `isLegacyHls() === true` → legacy
    - Chrome < 47 → legacy принудительно
    - webOS 1.x–2.x (WebKit, нет Chrome в UA) → legacy принудительно
    - иначе — по настройке
...
- [ ] Отдельным PR — включить автодетект Chrome-версии и сменить default.
  Опция «Старый телевизор» остаётся как escape hatch
