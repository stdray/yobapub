...
**Причина:** все рычаги выше декодера опровергнуты на устройстве (pasp ✗, CSS ✗, tkhd ✗, avc1 ✗ — плоскость рендера берёт размеры из SPS декодера), оставшиеся пути (нативный HLS платформы, транскод) не оправданы косметикой. Патчи, мутирующие init-сегмент, убраны целиком — «чтобы потом не сыграло»: при открытом инциденте sound-loss любые мутации контейнера должны быть исключены как фактор.

**Данные:** хронология попыток — три записи ниже; задача `anamorphic-aspect-tizen23` на доске work закрыта.

**Результат:** закрыто без фикса. Поля на 2.3 на анаморфных потоках — принятое поведение; не переоткрывать без нового рычага уровня декодера/платформы.

---

## 2026-06-11 — tkhd сдвинул videoWidth, но не плоскость рендера; попытка №3 — display width в avc1/stsd + pasp 1:1

**Решение:** в `patch-hls.js` добавлен патч `mp4-generator:avc1-display-width`: в avc1-боксе (stsd sample entry) width = coded × SAR (1280), а pasp нейтрализован в 1:1. Вместе с tkhd-патчем контейнер теперь везде согласованно описывает «квадратные пиксели, 1280×704»; реальные coded-размеры остаются только в SPS внутри avcC — их декодер и использует для декодирования.

**Причина:** после tkhd-патча `videoWidth` на Tizen 2.3 стал 1280 (WebKit берёт natural size из tkhd), но поля остались — плоскость рендера получает размеры из конфига декодера, который WebKit собирает из avc1/stsd (там оставалось 960). Это последний контейнерный рычаг: если и он не сработает — плоскость использует размеры из самого SPS (декодера), и контейнерные/CSS пути исчерпаны полностью; останутся только дорогие варианты (нативный HLS-плеер платформы через `video.src=m3u8` — DVB-пайплайн обязан уважать анаморф; транскод на проксе). Риск патча: некоторые строгие пайплайны могут не принять расхождение avc1 width (1280) и SPS (960) — следить за фатальными media-ошибками в легаси-режиме. Для Chromium-подобных нейтрально: natural = avc1 × pasp = 1280 × 1:1 — корректно.

**Данные:** сессии 8a4f/4577 (videoWidth=1280, поля на месте — слова юзера), патч-сайт `mp4-generator.js` строки ~4087 (width/hSpacing/vSpacing). Побочный эффект: parsePaspBox теперь всегда видит 1:1 → AspectFixer уходит в `none` (CSS-ветка на 2.3 всё равно не работает); телеметрия аспекта сводится к `videoWidth` в логе.

**Результат:** ОПРОВЕРГНУТО на устройстве (выкат `156d741` / 2.1.1-20, сессия 22:54Z): видео играет (avc1≠SPS пайплайн принял), `videoWidth=1280`, но поля на месте. ВЫВОД: плоскость рендера Tizen 2.3 берёт размеры из SPS самого декодера — контейнерные (pasp, tkhd, avc1) и CSS рычаги исчерпаны полностью. Дальше только: нативный HLS платформы (`video.src=m3u8`) для легаси-режима, транскод на проксе, либо принять как косметическое ограничение. Патчи tkhd/avc1 оставлены (безвредны, контейнер стал спек-корректнее).
...
**Результат:** ОПРОВЕРГНУТО (поспешно закрыл по логу — урок: лог `aspect none believed=1.8182` показывает только мнение WebKit, не плоскости рендера). На устройстве (выкат `4fbd87a` / 2.1.1-18, сессии 8a4f/4577): `videoWidth` стал 1280 — WebKit взял natural size из tkhd, но юзер подтвердил: поля остались. Вывод: аппаратная плоскость берёт размеры НЕ из tkhd (вероятно, из avc1/stsd-конфига декодера или из SPS). Следующая попытка — avc1 (запись выше). Побочная загадка: в сессии 15b0 (22:41Z) `aspect none sar=1:1 believed=1.8182 display=1.3636` — parsePaspBox увидел 1:1 при vw=1280; не объяснено, но безвредно (mode none).
...
**Результат:** подтверждено. Лог `olKGmIKXukaBpg_XihHQCg` — sn=2 больше не грузится, 1 bufferStalledError вместо ~10, время до стабильного воспроизведения ~4с вместо ~40с
