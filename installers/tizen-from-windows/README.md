...
Для обновления просто запустите `install.bat` снова — скрипт предложит скачать последнюю версию.

Сертификат хранится в папке `sdk-data\`. Пока эта папка на месте — обновления ставятся поверх, логин сохраняется.

**Важно:** если вы распаковали архив в новую папку (без `sdk-data\`), скрипт создаст новый сертификат. В этом случае старая версия будет автоматически удалена с ТВ и потребуется повторный логин.
