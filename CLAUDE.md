...
- **Prefer `.forEach` over indexed `for` for side-effect loops** — when a loop exists purely for side effects (binding handlers, updating DOM, logging) and has no early exit, use `arr.forEach(...)` instead of `for (let i = 0; i < arr.length; i++)`. Keep indexed `for` only when you need `break`/`continue`, or when the loop is on a hot path where per-iteration function allocation matters (e.g. per-frame player updates). For loops that build a string or array, prefer `.map().join('')` / `.filter().map()` over both `forEach` and `for` — see the immutability rule above.
- **Prefer the more compact form when stylistically equivalent** — when two syntaxes express the same thing with the same semantics, pick the shorter one. Examples: `if (x) f();` over a braced block for a single statement; `readonly foo: () => void` in interfaces over `foo(): void` only if there is a reason (otherwise shorthand wins on length); ternary over `if`/`else` for simple value selection; arrow function with implicit return over block body when the body is a single expression.
- **Arrow functions preferred** — use `const f = () => {}` syntax whenever possible for top-level functions, utility helpers, callbacks, templates. Arrow functions are more concise and modern. Avoid `function foo() {}` declarations. Example: prefer `export const tpl = (data: Data) => compiled(data)` over `export function tpl(data: Data) { return compiled(data); }`. **Exception: class methods.** Inside class bodies keep regular method syntax (`foo(): void { ... }`) — it is more compact than `readonly foo = (): void => { ... }`, lives on the prototype (one function per class instead of per instance), and `this` binds correctly for normal `this.foo()` calls. Only use an arrow-field in a class when the method is handed off as a callback that would otherwise lose `this` (e.g. `PageKeys.bind(this.handleKey)`).
