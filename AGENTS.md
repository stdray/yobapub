...
`src/back/YobaPub.Proxy/` — .NET reverse proxy with HLS manifest rewriting (`HlsRewriter.cs`). Config in `appsettings.json`. Smoke tests: `scripts/smoke.ps1 -BaseUrl <url>` (run locally and after every deploy; pass `-PetBoxApiKey` = key `yobapub-smoke` with `logs:query` to also verify events land in the PetBox `clients` log). PetBox integration lives in `PetBox/`: client logs relayed to the `clients` log in CLEF batches, self-logs via `Seq.Extensions.Logging` to `backend`, VIP list and client log levels resolved from PetBox config (`vip/logins`, `client-log/level`). The PetBox API key comes from env `PetBox__ApiKey`; with an empty key all PetBox integrations are disabled (local dev).
